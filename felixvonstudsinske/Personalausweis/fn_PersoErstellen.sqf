...
if !_PersoAendern then { // Neuer Perso
	if (SEL(_KostenIdentitaetNeu,0) > -1) then {
		if (((count FWFVS_Array_Personalausweise)+1) > SEL(_KostenIdentitaetNeu,0)) then {
			if (KONTO < SEL(_KostenIdentitaetNeu,1)) then {
				_Exit = true;
				hint format["Dir fehlen %1 € auf deinem Bankkonto, um eine neue Identität erstellen zu können.",ZAHLENKONVERTER(SEL(_KostenIdentitaetNeu,1) - KONTO)];
			} else {
				_Aktion = [
					format["Du bist dabei eine neue Identität zu erstellen. Dieser Vorgang kostet dich %1 € und werden von deinem Konto abgezogen.<br/>Aktuell hast du %2 € auf deinem Konto.<br/>Möchtest du eine weitere Identität erstellen?<br/><br/>Eine weitere Gegenfrage erfolgt nicht!",if (SEL(_KostenIdentitaetNeu,1) > 0) then {ZAHLENKONVERTER(SEL(_KostenIdentitaetNeu,1))}else{"keine"},ZAHLENKONVERTER(KONTO)],
					"Personalausweis kostenpflichtig erstellen",
					"FORTFAHREN",
					"ABBRECHEN"
				] call BIS_fnc_guiMessage;
				if(!isNil "_Aktion" && {!_Aktion}) exitWith {titleText["Erstellen abgebrochen","PLAIN"]; _Exit = true;};
	...
	_Aktion = [
		format["Du bist dabei deine aktuelle Identität zu verändern. Dieser Vorgang kostet dich %1 € und werden von deinem Konto abgezogen.<br/>Aktuell hast du %2 € auf deinem Konto.<br/>Der hinterlegte Name in dieser Identität kann nur in der Identitätsauswahl geändert werden!<br/>Möchtest du diese Identität bearbeiten?",if (SEL(_KostenIdentitaetNeu,1) > 0) then {ZAHLENKONVERTER(SEL(_KostenIdentitaetNeu,1))}else{"keine"},ZAHLENKONVERTER(KONTO)],
		"Personalausweis kostenpflichtig ändern",
		"FORTFAHREN",
		"ABBRECHEN"
	] call BIS_fnc_guiMessage;
	if(!isNil "_Aktion" && {!_Aktion}) exitWith {titleText["Änderung abgebrochen","PLAIN"]; _Exit = true;};
	_ZahlenBitte = SEL(_KostenIdentitaetNeu,1);
};
if (_Exit) exitWith {};
closeDialog 0;
...
perso_Mobilnummer = player getVariable "phone_ownNumber";
_MeinName STEXT format["Mein Name: %1",perso_Profilname];
...
} forEach ["maennlich","weiblich","divers"];

_Info STRUCT "Web-Link: <a href='http://www.hafensaenger-gaming.de/forum/'>Hafensaenger-Gaming</a>";
