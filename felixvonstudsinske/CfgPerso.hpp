...
class CfgPerso {
	class Einstellungen {
		...
		CivUngleicherName = 0;
		MaximaleIdentitaeten[] = {1,2,1,1}; // WEST - CIV(default) - INDP - EAST
		...
		KostenIdentitaetNameAendern[] = {0,250000000,0,0}; // WEST - CIV(default) - INDP - EAST - Kosten zum Ändern des Namens
		...
		Augenfarben[] = {"Blau","Grün","Braun","Grau"}; // Array: Augenfarben
		Staatsangehoerigkeit[] = {
			{"Australien","australisch"},
			{"Bruundi","bruundisch"},
			{"Dänemark","dänisch"},
			{"Deutschland","deutsch"},
			{"Frankreich","französich"},
			...
			{"Tschechische Republik","tschechisch"} // Siehe Exceltabelle "Perso - Staatsangehörigkeiten.ods"
		...
		class Identitaeten { // theoretisch unendlich viele Geschlechte möglich, zusätzlich im jeweiligen Script ändern, da sonst kein Aufruf
			class maennlich {
				class Adnet {
					...
					Pfad = "textures\Ausweis\Identitaeten\Adnet.paa"; // <-- Pfad zum Bild (jpg, paa, ..., kann auch Modfolder verwenden -> \addon\xx\pic.paa)
				};
				class Baros {
					Name = "Baros";
					Pfad = "textures\Ausweis\Identitaeten\Baros.paa";
				};
				class Byrne {
					Name = "Byrne";
					Pfad = "textures\Ausweis\Identitaeten\Byrne.paa";
				};
			};
			class weiblich {
				class Bikini_Frau {
					Name = "Ms Bikini";
					Pfad = "textures\Ausweis\Identitaeten\Bikini-Frau.paa";
				};
			};
			class divers {
				class divers {
	...
	class Orte {
		class Malden { // <-- Mapname hier eintragen
