...
if (playerSide isEqualTo west) then {
    ...
    for "_i" from 0 to 1 step 0 do {
       ...
       if (isNull _vehicle) exitWith {};
      _vehicle say3D ["siren2",250,1]; //Class name specified in description.ext, max distance & pitch
    ...
    for "_i" from 0 to 1 step 0 do {
       ...
       if (!alive _vehicle) exitWith {};
       if (isNull _vehicle) exitWith {};
      _vehicle say3D ["med2",75,1]; //Class name specified in description.ext, max distance & pitch
      sleep 1.555;//Exactly matches the length of the audio file.
      if (!(_vehicle getVariable "siren2")) exitWith {};
    };
};